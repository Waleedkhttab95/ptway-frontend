...
import './content.scss';
import {
	Table, Input, InputNumber, Button, Form, Modal, Cascader,
} from 'antd';
...
const {getAllMajors, getAllSubMajors, updateSubMajor, deleteSubMajor, addSubMajor} = majorContent;
...
الغاء
  	...
  	form.validateFields((error, row) => {
  		...
  		if (index > -1) {
  			...
  			this.setState({ data: newData, editingKey: ''}, async () => {
                  this.state.row.major !== row.major ? type= 'public_Major' : type='majorName' 
  ...
  showSubMajor = async ()=>{
        ...
        data.push({
            key: allSubMajor._id,
            name: allSubMajor.majorName,
            type: allSubMajor.key,
            major: allSubMajor.public_Major.majorName
        })
        this.setState({
            data
		});
...
render() {
	...
	return (
      <React.Fragment>
          ...
          <Cascader className="dropdown-menu major-dropmenu" options={majors} onChange={this.majorChange} placeholder="اختر التخصص" />
          <Button className="display-major-content" onClick={this.showSubMajor}>عرض</Button>
          <img src={add_icon} className="major-add-icon" alt="تخصص دقيق جديد" onClick={this.addSubMajorModal} />
          </div>
          <Modal
              title="اضافة مدينة جديدة"
        visible={this.state.visible}
        onOk={() => { this.handleOk((major)); }}
