...
import './content.scss';
import {
	Table, Input, InputNumber, Form, Modal,
} from 'antd';
...
const { getAllUniversities, updateUniversity, deleteUniversity, addUniversity } = universitiesContent;
...
الغاء
  ...
  handleOk= async () => {
  	...
  	await data.push({ name: addedUniversity.universtyName, key: addedUniversity._id });
  	this.setState({
  		data,
  		visible: false,
  	});
...
render() {
	...
	return (
      <React.Fragment>
          <img src={add_icon} className="add-icon" alt="جامعة جديدة" onClick={this.addCityModal} />
          <Modal
